...
def feed_a_overlap(query: Bucket, root: Bucket, feature_id: int,):
    ...
    if parent.feature_id == feature_id:
        ...
        tmp_overlap_buckets = dict()
        # 处理overlaps
        for oid in overlaps:
            bucket = parent.children[oid]
            # rtree实现的overlap似乎和我们的overlap有歧义，仅仅边相交的，我们不当作overlap
            if are_disjoint(bucket, query):
                continue
            overlap = get_overlap(query, bucket)  # 相交区域
            coordinates = overlap.mins+overlap.maxs
            tmp_contains = list(tmp_rtree.contains(coordinates))
            if tmp_contains:
                for bid in tmp_contains:
                    to_delete = tmp_overlap_buckets[bid][0]
                    tmp_rtree.delete(bid, to_delete.mins+to_delete.maxs)
                    del tmp_overlap_buckets[bid]
            else:
                tmp_overlap_buckets[oid] = [overlap, bucket]
                tmp_rtree.insert(oid, coordinates)
        del tmp_contains, tmp_rtree

        for [overlap, bucket] in tmp_overlap_buckets.values():
            checked_overlaps.append(overlap)
